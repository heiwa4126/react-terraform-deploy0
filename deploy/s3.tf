...
resource "aws_s3_object" "www" {
  for_each = fileset("${path.root}/../app1/dist-gz", "**/*")

  bucket = aws_s3_bucket.www.id
  key    = each.value
  source = "${path.root}/../app1/dist-gz/${each.value}"

  etag             = filemd5("${path.root}/../app1/dist-gz/${each.value}")
  content_type     = lookup(local.mime_types, regex("\\.[^.]+$", each.value), null)
  content_encoding = contains(tolist([".js", ".svg", ".css"]), regex("\\.[^.]+$", each.value)) ? "gzip" : null
